...
Vcheck=mean((JD-M).^2); %finding variance by subtracting the mean from the data set, squaring it, then taking the mean of that vector to get a scalar number
...
meanNN; %mean of each bucket of 512 point length record
meanmeanNN=mean(meanNN); %mean of vector of means
vNN; %variance of each bucket of 512 length record
meanvNN=mean(vNN); %mean of vector of variances
RMSNN; %RMS of each bucket of 512 length record
meanRMSNN=mean(RMSNN); %mean of vector of RMSs
...
%find c (remove the mean of each sample record from each sample record-----

%load data with the mean (i.e. don't subtract it out)
load jet1.dat
JD=jet1;
JDcalc=jet1;
load Gaussian.txt
...
for i=1:NN                  %index from 1 to the number of bin being created
    JDtemp=JDcalc(1+(i-1)*Len:i*Len); %for index 1, the bin will go from 1 to L. For index 2, the bin will go from L+1 to 2L 
    meanNN(i)=mean(JDtemp); %finds mean of first segment of data on first pass and will report numbers that populate a 1x38 array
    JDtemp=JDtemp-meanNN(i); %subtracts out mean from each sample record
    vNN(i)=var(JDtemp,1); %finds variance of data along each segment 
    RMSNN(i)=sqrt(vNN(i));
    A=fft(JDtemp); %do fast fourier transform
    AA=conj(A).*A; %calculate the conjugate, has to be .* to make sure its element by element
    ...
    Saa_avg=Saa_avg+Saatemp; %write over and store the information for the fft from the for loop
        ...
        for j=1:Len
           ...
           for t=1:Len
              ...
              Raamsr(i,j)=Raamsr(i,j)+JDtemp(t)*JDtemp(k);
           end
        end
end
Saamsr=Saa_avg/NN; %Saa for case with 38 bins
fMaxRes=fs/Len; %max resolution frequency 
fmsr=linspace(0,fs-fMaxRes,Len); %makes the x-axis for frequency vectors

Raamsr=Raamsr./Len; %normalize the Raa vector by length N
Raamavg=mean(Raamsr,1);
Raamavg=Raamavg./max(Raamavg);
fmsr1=linspace(0,Len/fs,Len); 
...
semilogy(f,Saa1,'g','DisplayName','Data without mean') %plot for spectral data with 512 length without mean
title('Loglog autospectral density data')
xlabel('f (Hz)')
ylabel('Suu (m^2/s^3)')
hold on
semilogy(fm,Saam1,'m','DisplayName','Data with mean') %plot for spectral data with 512 length with mean
semilogy(fmsr,Saamsr,'y','DisplayName','Data with mean of each sample record from each sample record removed')
legend('show')
xlim([0 50])
hold off
